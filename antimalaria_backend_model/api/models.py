...
class ModelUpdate(models.Model):
    ...
    def __str__(self):
        return f"Update for {self.model.name} by {self.user.username} on {self.created_at}"
...
class PredictionCompound(models.Model):
    ...
    class Meta:
        # This constraint ensures you don't save a result for the same compound twice within the same prediction job.
